...
def criaServidoresJMS(propriedades):
    for server in servidores:
        nomeServidor = propriedades.get(server)
        print 'Criando servidor jms para o server ==>> ' + nomeServidor
        cd('/')
        cmo.createJMSServer('JMSServer-'+nomeServidor)
        cd('/Deployments/JMSServer-'+nomeServidor)
        set('Targets',jarray.array([ObjectName('com.bea:Name='+nomeServidor+',Type=Server')], ObjectName))
    print 'Servidores JMS criados!!!'
...
criaServidoresJMS(propriedades)
